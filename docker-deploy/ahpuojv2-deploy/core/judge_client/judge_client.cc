...
void _update_user_mysql(int &user_id)
{
	char sql[BUFFER_SIZE];
	sprintf(sql,
			"UPDATE `user` SET `solved`=(SELECT count(DISTINCT `problem_id`) FROM `solution` WHERE `id`=\'%d\' AND `result`=\'4\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
			user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
	sprintf(sql,
			"UPDATE `user` SET `submit`=(SELECT count(*) FROM `solution` WHERE `id`=\'%d\' AND `problem_id>\'0\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
			user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
}
