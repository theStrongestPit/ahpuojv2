...
// 此处有修改 逻辑修改为只统计非比赛中的提交
void _update_user_mysql(int &user_id)
{
	char sql[BUFFER_SIZE];
	sprintf(sql,
					"UPDATE `user` SET `solved`=(SELECT count(DISTINCT `problem_id`) FROM `solution` WHERE `user_id`=\'%d\' AND `result`=\'4\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
					user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
	sprintf(sql,
					"UPDATE `user` SET `submit`=(SELECT count(*) FROM `solution` WHERE `user_id`=\'%d\' AND `problem_id`>\'0\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
					user_id, user_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
}
...
#ifdef _mysql_h
// 此处有修改 逻辑修改为只统计非竞赛中的提交
void _update_problem_mysql(int p_id)
{
	char sql[BUFFER_SIZE];
	sprintf(sql,
					"UPDATE `problem` SET `accepted`=(SELECT count(*) FROM `solution` WHERE `problem_id`=\'%d\' AND `result`=\'4\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
					p_id, p_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
	sprintf(sql,
					"UPDATE `problem` SET `submit`=(SELECT count(*) FROM `solution` WHERE `problem_id`=\'%d\' AND `contest_id`= \'0\') WHERE `id`=\'%d\'",
					p_id, p_id);
	if (mysql_real_query(conn, sql, strlen(sql)))
		write_log(mysql_error(conn));
}
...
void _get_problem_info_mysql(int p_id, int &time_lmt, int &mem_lmt,
														 int &isspj)
{
	printf("_get_problem_info_mysql\n");
...
void copy_go_runtime(char *work_dir)
{
	...
	// execute_cmd("/bin/cp /usr/lib/go/bin/go %s/usr/bin/", work_dir);
										 ...
										 int &runner_id)
{
	...
	if (argc > 3)
		...
		strcpy(oj_home, "/home/judge/hustoj_core"); // 此处有修改
