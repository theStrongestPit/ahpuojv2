...
select src.solution_id,src.source from jol.solution s inner join jol.source_code src on s.solution_id = src.solution_id where s.result != 13;

truncate table oj.solution;
insert into oj.solution (solution_id,problem_id,team_id,user_id,contest_id,num,time,memory,in_date,result,language,ip,judgetime,valid,code_length,pass_rate,lint_error,judger)
select os.solution_id,os.problem_id-1000,0,os.id,0,os.num,os.time,os.memory,os.in_date,os.result,os.language,os.ip,os.judgetime,1,os.code_length,os.pass_rate,os.lint_error,os.judger from results os;

DROP TEMPORARY TABLE IF EXISTS results;

truncate table oj.source_code;
insert into oj.source_code
select src.solution_id,src.source,0 from src;
delete from source_code where solution_id not in (select solution_id from solution)

update solution set num=0 where num=-1
